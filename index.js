...
async function checkBalance(userSigner) {
...
bot.on('message', async (ctx) => {
    ...
    try {
        ...
        db.get("SELECT * FROM users WHERE userId = ?", [userId], async (err, row) => {
            ...
            // Mint the NFT
            try {
                await prepareMintNFT(imageFilePath, "User's NFT", userSigner, ctx);
            } catch (err) {
...
async function prepareMintNFT(filePath, name, userSigner, ctx) {
    ...
    // Store the userContract instance by userId
    const userId = String(ctx.from.id);
    userSigners[userId] = userSigner;
    userContracts[userId] = userContract;
    userMetadataLinks[userId] = metadataLink;

    
    const contract = new web3.eth.Contract(nftContractAbi, contractAddress);
    async function estimateMintGas(address, metadataLink) {
        try {
            const gasEstimate = await contract.methods.mint(address, metadataLink).estimateGas({ from: address });
            return gasEstimate;
        } catch (error) {
    ...
    const balanceInWei = await provider.getBalance(userSigner.address);
    console.log(weiToEther(balanceInWei))

    const gasPrice = await web3.eth.getGasPrice();
    const gasEstimate = await estimateMintGas(userSigner.address, metadataLink);
    const totalCost = BigInt(gasEstimate) * BigInt(gasPrice);
    
    userHasConfirmed[userId] = false;

    const msg = `
Gas Estimate: ${gasEstimate}
Gas Price: ${gasPrice}
Total Cost (in wei): ${totalCost}
Total Cost (in ether): ${web3.utils.fromWei(totalCost.toString(), 'ether')}
Do you want to proceed with this transaction?`;

    const inlineKeyboard = {
        reply_markup: {
            inline_keyboard: [
                [{ text: 'Yes', callback_data: 'confirm_mint' }, { text: 'No', callback_data: 'cancel_mint' }]
            ]
        }
    };
    
    await ctx.reply(msg, inlineKeyboard);
    
    // Set a timeout to automatically cancel if not confirmed in 2 minutes (you can adjust this)
    setTimeout(() => {
        if (!userHasConfirmed[userId]) {
            delete userHasConfirmed[userId];
            ctx.reply('Transaction timeout. Please try again.');
        }
    }, 2 * 60 * 1000);
}
...
bot.on('callback_query', async (ctx) => {
    const userId = String(ctx.from.id);
    const metadataLink = userMetadataLinks[userId];
    if (!metadataLink) {
        console.error("Metadata link not found for user:", userId);
        ctx.reply("An error occurred. Please try again.");
        return;
    }
    const userSigner = userSigners[userId];
    if (!userSigner) {
        console.error("User signer not found for user:", userId);
        ctx.reply("An error occurred. Please try again.");
        return;
    }
    const userContract = userContracts[userId];
    if (!userContract) {
        console.error("User contract not found for user:", userId);
        ctx.reply("An error occurred. Please try again.");
        return;
    }
    const callbackData = ctx.callbackQuery.data;

    if (callbackData === 'confirm_mint') {
        const userId = String(ctx.from.id);
        userHasConfirmed[userId] = true;
    ...
    } else if (callbackData === 'cancel_mint') {
        ctx.reply('You cancelled the transaction.');
    }
});
